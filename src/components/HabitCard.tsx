...
export default function HabitCard({ habit, onClick, onAdd }: HabitCardProps) {
  return (
    <Card onClick={onClick} className="relative">
      <div className={`absolute inset-0 opacity-10 ${habit.colorClass}`} style={{ width: `${habit.progress}%` }} />
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-end">
            ...
            <span className="text-3xl font-headline font-black text-primary">
              {habit.current} <span className="text-xs font-medium text-on-surface-variant tracking-normal">/ {habit.goal} {habit.unit}</span>
            </span>
          </div>
